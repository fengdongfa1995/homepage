...
dagum_gini <- function(model, df){
  ...
  for (j in group) {
    for (h in group){
      ...
      if (j == h){
        within_gini <- within_gini + item
      } else {
        d_jh = relative_economic_affluence(income_j, income_h)
        net_between_gini <- net_between_gini + item * d_jh
        trans_gini <- trans_gini + item * (1 - d_jh)
      }
    }
  }
  
  c(G=total_gini_index, Gw=within_gini, Gnb=net_between_gini, Gt=trans_gini)
}
```
